...
async function importFromLDAP() {
    ...
    let created = 0, updated = 0, disabled = 0, missing = 0, issuesCreated = 0;
    const errors = [];
    ...
    db.prepare(
        `UPDATE sync_log SET finished = datetime('now', 'localtime'),
         users_created = ?, users_updated = ?, users_disabled = ?, users_missing = ?, errors = ?
         WHERE id = ?`
    ).run(created, updated, disabled, missing, JSON.stringify(errors), logId);
